"""OpenAI LLM client with context management and intelligent truncation."""
...
class ConversationContext:
    ...
    def get_messages_for_api(self) -> List[Dict[str, str]]:
        ...
        for message in self.messages:
            api_messages.append(message.to_openai_format())
...
class OpenAILLMClient(BaseResilientClient[LLMResponse]):
    """
    OpenAI LLM client with context management and intelligent truncation.
    ...
    async def generate_response(
        ...
        async def _make_request() -> LLMResponse:
            ...
            try:
                ...
                self.logger.warning(f"Rate limit exceeded: {e}", extra={"correlation_id": correlation_id})
                raise
            except openai.APITimeoutError as e:
                self.logger.warning(f"API timeout: {e}", extra={"correlation_id": correlation_id})
                raise
            except openai.APIError as e:
                self.logger.error(f"OpenAI API error: {e}", extra={"correlation_id": correlation_id})
                raise
            except Exception as e:
                self.logger.error(f"Unexpected error: {e}", extra={"correlation_id": correlation_id})
                raise
    ...
    async def stream_response(
        ...
        """Stream response from OpenAI API for reduced latency."""
        if correlation_id is None:
            correlation_id = self._generate_correlation_id()
