...
class Settings(BaseSettings):
    ...
    livekit_webhook_secret: Optional[str] = Field(
        default=None,
        description="LiveKit webhook secret for signature validation"
    )
    ...
    groq_api_key: Optional[str] = Field(
        default=None,
        description="Groq API key for language model"
    )
    ...
    cartesia_voice_id: str = Field(
        default="064b17af-d36b-4bfb-b003-be07dba1b649",  # Valid Cartesia voice ID
            ...
            try:
                if not validate_secret_key(v):
                    raise ValueError(
                        ...
                        f"Must be at least 32 characters long and cryptographically strong."
                    )
            except Exception:
                # If validation fails due to missing security module, allow in non-production
                if info.data.get('environment') == Environment.PRODUCTION:
                    raise ValueError("Secret key validation failed - ensure proper security configuration")
    ...
    def validate_deepgram_key(cls, v, info):
        ...
        if v is not None and v.strip():
            ...
            try:
                ...
                # If validation fails due to missing security module, allow in non-production
                if hasattr(info, 'data') and info.data and info.data.get('environment') == Environment.PRODUCTION:
                    raise ValueError(f"Deepgram API key validation failed: {e}")
        return v
    ...
    def validate_cartesia_key(cls, v, info):
        ...
        if v is not None and v.strip():
            ...
            try:
                ...
                # If validation fails due to missing security module, allow in non-production
                if hasattr(info, 'data') and info.data and info.data.get('environment') == Environment.PRODUCTION:
                    raise ValueError(f"Cartesia API key validation failed: {e}")
        return v
    ...
    def validate_livekit_key(cls, v, info):
        ...
        if v is not None and v.strip():
            ...
            try:
                ...
                # If validation fails due to missing security module, allow in non-production
                if hasattr(info, 'data') and info.data and info.data.get('environment') == Environment.PRODUCTION:
                    raise ValueError(f"LiveKit API key validation failed: {e}")
        return v
    ...
    def validate_groq_key(cls, v, info):
        ...
        if v is not None and v.strip():
            ...
            try:
                result = validate_api_key(v, APIKeyType.GROQ)
                if not result.is_valid:
                    raise ValueError(f"Invalid Groq API key: {result.error_message}")
            except Exception as e:
                # If validation fails due to missing security module, allow in non-production
                if hasattr(info, 'data') and info.data and info.data.get('environment') == Environment.PRODUCTION:
                    raise ValueError(f"Groq API key validation failed: {e}")
        return v
    ...
    def validate_production_requirements(self):
        ...
        if self.environment == Environment.PRODUCTION:
            required_fields = {
                ...
                'groq_api_key': self.groq_api_key,  # Changed from openai_api_key to groq_api_key
    ...
    try:
        ...
        return {
            ...
            'required_services': {
                ...
                'groq': bool(settings.groq_api_key),  # Changed from openai to groq
