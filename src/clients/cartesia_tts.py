...
class AudioConfig:
    ...
    def to_cartesia_format(self) -> Dict[str, Any]:
        ...
        if self.format == AudioFormat.MP3 and self.bit_rate:
            output_format["bit_rate"] = self.bit_rate
        elif self.format == AudioFormat.RAW and self.encoding:
            output_format["encoding"] = self.encoding.value
...
class CartesiaTTSClient(BaseResilientClient[TTSResponse]):
        ...
        async def _stream_synthesis() -> AsyncIterator[bytes]:
            ...
            try:
                ...
                self.logger.error(
                    ...
                    extra={"correlation_id": correlation_id, "text_preview": processed_text[:50]}
                )
                self.usage_stats.add_failed_request()
                raise
    ...
    async def health_check(self) -> bool:
        ...
        try:
            ...
            # Try batch synthesis with short timeout
            response = await self.synthesize_batch(
                text=test_text,
                voice_config=voice_config,
                audio_config=AudioConfig(format=AudioFormat.WAV, sample_rate=16000)
            )
            
            # Check if we got valid audio data
            return len(response.audio_data) > 1000  # Should have substantial audio data
